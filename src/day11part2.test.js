...
function monkeyBusiness(monkeysString) {
  ...
  range(10000).forEach(() => {
    monkeys.forEach((m) => m.playRound(monkeys, divisor));
  });

  console.log(`inspections: ${monkeys.map((m) => m.getItemsInspected())}`);
...
function Monkey(id, items, operation, test) {
  ...
  function playRound(monkeys, divisor) {
    for (const item of getItems()) {
      itemsInspected++;
      const newWorryLevel = eval(operation.replaceAll("old", item)) % divisor;
  ...
  return { id, getItems, playRound, getItemsInspected, test };
}
